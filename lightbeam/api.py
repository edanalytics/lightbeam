...
class EdFiAPI:
    ...
    def prepare(self):
        ...
        try:
            ...
            api_base = requests.get(
                self.config["base_url"],
                verify=self.lightbeam.config["connection"]["verify_ssl"],
            )
        except Exception as e:
            self.logger.critical(
                "could not connect to {0} ({1})".format(self.config["base_url"], str(e))
            )
        ...
        try:
            ...
            self.logger.critical(
                "could not parse response from {0} ({1})".format(
    ...
    def apply_filters(self, endpoints=[]):
        # apply filters
        my_endpoints = util.apply_selections(
            endpoints, self.lightbeam.selector, self.lightbeam.exclude
        )

        # make sure we have some endpoints to process
        if not my_endpoints:
            self.logger.critical(
                "selector filtering left no endpoints to process; check your selector for typos?"
            )
        ...
        if unknown_endpoints:
            self.logger.critical(
                "no match for selector(s) [{0}] to any endpoint in your API; check for typos?".format(
    ...
    def get_retry_client(self):
        return RetryClient(
            timeout=aiohttp.ClientTimeout(
                sock_connect=self.lightbeam.config["connection"]["timeout"]
            ),
            retry_options=ExponentialRetry(
                attempts=self.lightbeam.config["connection"]["num_retries"],
                factor=self.lightbeam.config["connection"]["backoff_factor"],
                statuses=self.lightbeam.config["connection"]["retry_statuses"].append(
                    401
                ),
            ),
            connector=aiohttp.connector.TCPConnector(
                limit=self.lightbeam.config["connection"]["pool_size"]
            ),
        )
    ...
    def do_oauth(self):
        try:
            ...
            self.logger.error(
                f"OAuth token could not be obtained; check your API credentials?"
            )
    ...
    def get_data_url(self):
        if self.config["version"] != 3:
            self.logger.critical(
                "Sorry, lightbeam only supports connections to v3+ Ed-Fi APIs."
            )
        if self.config["base_url"][-1] != "/":
            url = self.config["base_url"] + "/" + "data/v3"
        else:
            url = self.config["base_url"] + "data/v3"
        ...
        if self.config["mode"] is None:
            ...
            if (
                "year" not in self.config.keys()
                or "instance_code" not in self.config.keys()
            ):
                self.logger.critical(
                    "`instance_code` and `year` required for 'instance_year_specific' mode."
                )
            url += "/" + self.config["instance_code"] + "/" + str(self.config["year"])
        else:
            self.logger.critical(
                f"Invalid `api_mode` - must be one of: [shared_instance, sandbox, "
    ...
    def get_sorted_endpoints(self):
        ...
        try:
            response = requests.get(
                self.config["dependencies_url"],
                verify=self.lightbeam.config["connection"]["verify_ssl"],
            )
            if response.status_code not in [200, 201]:
                raise Exception(
                    "Dependencies URL returned status {0} ({1})".format(
                        response.status_code,
                        (
                            (response.content[:75] + "...")
                            if len(response.content) > 75
            ...
            self.logger.critical(
                "Unable to load dependencies from API... terminating. Check API connectivity. ({0})".format(
                    str(e)
                )
            )
    ...
    def load_swagger_docs(self):
        ...
        try:
            ...
            response = requests.get(
                self.config["open_api_metadata_url"],
                verify=self.lightbeam.config["connection"]["verify_ssl"],
            )
            if not response.ok:
                raise Exception(
                    "OpenAPI metadata URL returned status {0} ({1})".format(
                        response.status_code,
                        (
                            (response.content[:75] + "...")
                            if len(response.content) > 75
            ...
            self.logger.critical(
                "Unable to load Swagger docs from API... terminating. Check API connectivity."
            )
        ...
        for endpoint in openapi:
            ...
            if endpoint_type == "descriptors" or endpoint_type == "resources":
                ...
                if self.lightbeam.track_state:
                    ...
                    file = os.path.join(
                        cache_dir, f"swagger-{endpoint_type}-{hash}.json"
                    )
                if (
                    ...
                    and time.time() - os.path.getmtime(file)
                    < self.SWAGGER_CACHE_TTL  # cache file isn't expired
                ):
                    self.logger.debug(
                        f"re-using cached {endpoint_type} swagger doc (from {file})..."
                    )
                    with open(file) as f:
                        swagger = json.load(f)
                else:
                    ...
                    try:
                        ...
                        self.logger.critical(
                            f"Unable to load {endpoint_type} Swagger from API... terminating. Check API connectivity."
                        )
    ...
    async def load_descriptors_values(self):
        ...
        if (
            ...
            < self.DESCRIPTORS_CACHE_TTL  # the cache file isn't expired
        ):
            # cache file exists and we can use it!
            self.logger.debug(
                f"re-using cached descriptor values (from {cache_file})..."
            )
    ...
    def get_params_for_endpoint(self, endpoint):
        if "Descriptor" in endpoint:
            swagger = self.descriptors_swagger
        else:
            swagger = self.resources_swagger
        definition = (
    ...
    def get_required_params_from_swagger(self, swagger, definition, prefix=""):
        ...
        for requiredProperty in schema["required"]:
            if "$ref" in schema["properties"][requiredProperty].keys():
                ...
                sub_params = self.get_required_params_from_swagger(
                    swagger, sub_definition, prefix=requiredProperty + "."
                )
