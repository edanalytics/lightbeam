...
class Deleter:
    ...
    def delete(self):
        ...
        if not self.lightbeam.config.get("force_delete", False):
            if (
                input(
                    'Type "yes" to confirm you want to delete payloads for the selected endpoints? '
                )
                != "yes"
            ):
        ...
        for endpoint in endpoints:
            ...
            if endpoint == "students":
                self.logger.warn(
                    "data for {0} endpoint cannot be deleted (this is an Ed-Fi limitation); skipping".format(
                        endpoint
                    )
                )
                continue
    ...
    async def do_deletes(self, endpoint):
        ...
        if endpoint.endswith("Descriptors"):
            self.logger.info(
                "fetching current descriptors from endpoint {0} ...".format(endpoint)
            )
            ...
            try:
                if endpoint.endswith("Descriptors"):
                    ...
                    matching_descriptors = [
                        x
                        for x in self.lightbeam.results
                        if x["namespace"] == params["namespace"]
                        and x["codeValue"] == params["codeValue"]
                    ]
                    if len(matching_descriptors) == 1:
                        the_id = matching_descriptors[0]["id"]
                        await self.do_delete_id(endpoint, the_id, file_name, line)
                        break
                    elif len(matching_descriptors) > 1:
                        ...
                        if status != 401:
                            ...
                            if status in [200, 201]:
                                ...
                                if type(j) == list and len(j) == 1:
                                    ...
                                    await self.do_delete_id(
                                        endpoint, the_id, file_name, line
                                    )
                                    break
                                    ...
                                    skip_reason = (
                                        "multiple matching payloads found in API"
                                    )
                                else:
                ...
                self.logger.exception(
                    e, exc_info=self.lightbeam.config["show_stacktrace"]
                )
                self.logger.error(
                    "  (at line {0} of {1}; ID: {2} )".format(line, file_name, id)
                )
                break
