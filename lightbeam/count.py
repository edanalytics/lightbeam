...
class Counter:
    ...
    def count(self):
        ...
        self.lightbeam.results = sorted(
            self.lightbeam.results, key=lambda x: sort_keys.index(x[0])
        )
        ...
        if self.lightbeam.results_file:
            ...
            with open(self.lightbeam.results_file, "w") as fp:
                ...
                fp.write(
                    "Records"
                    + self.lightbeam.config["count"]["separator"]
                    + "Endpoint\n"
                )
        ...
        try:
            ...
            async with self.lightbeam.api.client.get(
                ...
                if status not in ["200", "201"]:
                    self.logger.warn(
                        f"Unable to load counts for {endpoint}... {status} API response."
                    )
                else:
            ...
            self.logger.critical(
                f"Unable to load counts for {endpoint} from API... terminating. Check API connectivity."
            )
